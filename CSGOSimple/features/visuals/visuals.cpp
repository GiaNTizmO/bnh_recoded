...
void c_visuals::Menu() {
	...
	ImGui::BeginChild("##1st", ImVec2(-1, 205), true); {
		...
		ImGui::Checkbox("SnapLines", &other.snipeline);
		ImGui::Checkbox("Head Dot", &other.head_dot);
		...
		if (ImGui::BeginPopup("##flagspopup")) {
			...
			if (ImGui::IsItemHovered()) {
				ImGui::SameLine();
				ImGui::Text(" | Note for D3F0LT: HK = HELMET/KEVLAR");
			}
	...
	ImGui::BeginChild("##second", ImVec2(-1, 195), true); {
		...
		ImGui::Checkbox("Glow Bloom", &glow.bloom);
	...
	ImGui::BeginChild("##third", ImVec2(-1, -1), true); {
		...
		ImGui::ColorEdit("HP Bar", other.hp_clr);
		...
		ImGui::ColorEdit("SnapLines", other.snipeline_clr);
		ImGui::ColorEdit("Head Dot", other.head_dot_clr);
