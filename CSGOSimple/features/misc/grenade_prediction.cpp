...
namespace GrenadePrediction
{
	...
	void draw( ) {
		...
		if( _predicted ) {
			for( auto &p : _points ) {
				...
				if( valid && valid2 ) {
					Render::Get( ).RenderLine( start.x, start.y, end.x, end.y, Color(g_Misc->colors.grenade_prediction_color), 1.5f );
					//if( p.m_plane ) {
					...
					//}
					if(p.m_detonate)
						Render::Get().RenderCircle3D(p.m_end, 100.f, 150.f, g_Misc->colors.grenade_prediction_color);
				}
			}
		}
	}
	...
	void predict( CUserCmd *ucmd ) {
		...
		for( auto time = 0.0f; index < 500; time += step ) {
			...
			const short weapon_index = weapon->m_Item( ).m_iItemDefinitionIndex( );
	...
	bool detonated( C_BaseCombatWeapon* weapon, float time, trace_t &trace ) {
		...
		const short index = weapon->m_Item( ).m_iItemDefinitionIndex( );
