    ...
    if (first_begin_of_the_frame)
    {
        ...
        else
        {
            ...
            if ((!(flags & ImGuiWindowFlags_ChildWindow) && !(flags & ImGuiWindowFlags_Popup) && !(flags & ImGuiWindowFlags_Tooltip))) {
                ImColor b_col = GetColorU32(ImGuiCol_WindowBg);
                ImU32 bg_col = ImColor(33 / 255.f, 33 / 255.f, 33 / 255.f, g.Style.Alpha < b_col.Value.w ? g.Style.Alpha : b_col.Value.w);
                float alpha = 1.0f;
                ...
                auto acc_col = g_Misc->colors.accent_color;
                if (acc_col.a() > g.Style.Alpha)
                    acc_col.SetColor(acc_col.r(), acc_col.g(), acc_col.b(), (int)g.Style.Alpha);
                
                if (!(flags & ImGuiWindowFlags_Notification)) {
                    window->DrawList->AddRect(window->Pos , window->Pos + window->Size, ImColor(acc_col.r() / 255.f, acc_col.g() / 255.f, acc_col.b() / 255.f, g.Style.Alpha));
                }
                else {
                    RenderFrame(window->Pos, ImVec2(window->Pos.x - 1, window->Pos.y + window->Size.y), ImColor(acc_col.r() / 255.f, acc_col.g() / 255.f, acc_col.b() / 255.f, g.Style.Alpha < b_col.Value.w ? g.Style.Alpha : b_col.Value.w));
                }
            }
            else {
                if (flags & ImGuiWindowFlags_ChildWindow && !(flags & ImGuiWindowFlags_Tooltip)) {
                    ...
                    if (!(flags & ImGuiWindowFlags_NoTitleBar))
                        ...
                        textSize = ImVec2(-10, 0);

                    ImU32 bg_col = ImColor(27 / 255.f, 27 / 255.f, 27 / 255.f, g.Style.Alpha);
                    float alpha = 1.0f;
                    if (g.NextWindowData.BgAlphaCond != 0)
                        alpha = g.NextWindowData.BgAlphaVal;
                    if (alpha != 1.0f)
                        bg_col = (bg_col & ~IM_COL32_A_MASK) | (IM_F32_TO_INT8_SAT(alpha) << IM_COL32_A_SHIFT);
                    if (!(flags & ImGuiWindowFlags_DoNotFillBG)) {
                        window->DrawList->AddRectFilled(window->Pos + ImVec2(0, window->TitleBarHeight()), window->Pos + window->Size, bg_col, window_rounding, (flags & ImGuiWindowFlags_NoTitleBar) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Bot);
        ...
        window->DC.NavHasScroll = (GetScrollMaxY(window) > 0.0f);
        ...
        //	if (flags & ImGuiWindowFlags_NoTitle)
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (*v)
    {
        ...
        //RenderFrame(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), GetColorU32(ImGuiCol_CheckMark));
    }
